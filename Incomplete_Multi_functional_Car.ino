...
/*
 * IR_control: no parameeters
 * pressing # (IR_ESC) on the IR remote stops IR mode
 * "Algorithm":
 * wait for 100ms (to process)
 * check what key was pressed and act accordingly ( all arrows, OK, keys 1-4)
 *        make sure to check if serial port receives the data 
 *        using irrecv.decod(&results) before any action
 * recieve the next value via irrecv.resume()  
 * 
 */
